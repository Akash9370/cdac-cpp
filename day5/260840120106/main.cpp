...
void displayAccountsByType(Account* bank[], int total) {
    int typeChoice;
    cout << "\nSelect Account Type to Display:\n";
    ...
    for (int i = 0; i < total; ++i) {
        if (bank[i] == nullptr) continue;

        if (typeChoice == 1) {
            // Attempt to dynamic_cast base pointer to SavingsAccount pointer
            SavingsAccount* savAcc = dynamic_cast<SavingsAccount*>(bank[i]);
            if (savAcc != nullptr) {
                savAcc->display();
                foundCount++;
            }
            ...
            // Attempt to dynamic_cast base pointer to CurrentAcc pointer
            CurrentAcc* currAcc = dynamic_cast<CurrentAcc*>(bank[i]);
            if (currAcc != nullptr) {
                currAcc->display();
                foundCount++;
            }
            ...
            // Attempt to dynamic_cast base pointer to DematAccount pointer
            DematAccount* dematAcc = dynamic_cast<DematAccount*>(bank[i]);
            if (dematAcc != nullptr) {
                dematAcc->display();
                foundCount++;
            }
    ...
    if (foundCount == 0) {
        cout << "No accounts of the selected type were found.\n";
    } else {
        cout << "\nTotal accounts displayed: " << foundCount << "\n";
    }
}
...
int main() {
    ...
    do {
        ...
        cout << "7. Change PIN\n";
        cout << "8. Display Accounts by Type\n";    
        cout << "0. Exit\n";
        cout << "Select Option: ";
        ...
        switch (choice) {
            case 1: {
                ...
                if (typeChoice == 1) { // Savings Account
                    int minBal = SavingsAccount::MIN_BAL_saving;
                    if (bal < minBal) {
                        cout << "Creation failed! Savings Account requires a minimum deposit of " << minBal << "    .\n";
                        break;
                    }
                    char chq[50];
                    ...
                    int minBal = CurrentAcc::MIN_BAL_current;
                    if (bal < minBal) {
                        cout << "Creation failed! Current Account requires a minimum deposit of " << minBal << ".\n";
                        break;
                    }
                    int tpd;
            ...
            case 8: {
                displayAccountsByType(bank, totalAccounts);
                break;
            }
            case 0:
